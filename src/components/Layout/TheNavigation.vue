<template>
    <header class="h-18 grid grid-cols-12 justify-between  auto-rows-auto px-[15px] md:px-[30px] lg:px-[100px] mb-3 ">
        <div class="col-span-9 sm:col-span-3 md:col-span-3 xl:col-span-2 flex items-center justify-start">
            <img src="../../assets/icons/bxs-plane-alt 1MyDreamPlace.svg" alt="My Dream Place Icon" title="My Dream Place">
            <p class="text-lg  font-medium leading-5">
                my Dream Place
            </p>
        </div>

        <nav class="hidden sm:flex items-center sm:col-start-5 sm:col-end-11 md:col-start-4 md:col-end-10 lg:col-end-10 xl:col-start-4 xl:col-end-10 ">
            <ul 
            class="w-full sm:flex xl:justify-center justify-between content-center p-[5px]">
                <li  v-for="(listItem,index) in NAVIGATION_LINKS" :key="listItem" class="p-0 h-5 xl:mx-6 text-base font-normal leading-5">
                    {{ listItem }}
                </li>
            </ul>
        </nav>

        <div v-if="!hasToken" class="hidden sm:col-start-11 bg-yellow sm:col-end-13 md:col-start-11 md:col-end-13 xl:col-start-12 sm:flex justify-end items-center">
    ...
    <ul v-if="showDropDown" class="w-full px-[15px]">
